...
% Perform correlation between the zc symbol 4 (including its CP) and the
% signal
...
[~, samp1] = max(abs(corr1));
samp1 = samp1-length(zc_seq_sym_4)+1;
zc_4_cand = signal(samp1:samp1+ofdm_sym_len-1);
mult = zc_4_cand .* conj(zc_seq_sym_4);
fft_zc_cand = fftshift(fft(zc_4_cand));
figure;
plot(abs(fft_zc_cand))
freq_offset = 
