...
function raw_hex = physical_layer_demod(raw_samples)
    ...
    % % Channel estimation
    % synced_samples_with_channel_estimation = fix_channel_estimation(synced_samples, zc_symbol_4, zc_symbol_6);
...
% TESTED
function demod = ofdm_demod(sig, zc_symbol_4, zc_symbol_6)
    cp_len = 72;
    cp_len_extended = 80;
    ...
    % Divide by the channel in the frequency domain

    [zc_symbol_4, zc_symbol_6] = create_zc_symbols();

    H = find_channel_in_freq(sig, zc_symbol_4, zc_symbol_6);
    demod2_8 = demod2_8 ./ repmat(H ,7, 1);
    demod9 = demod9 ./ H;
...
function H = find_channel_in_freq(signal, zc_symbol_4, zc_symbol_6)
    
    ofdm_symb_len = 1024;
    cplen = 72;
    cplen_extended = 80;
    num_of_ofdm_symbols = 9;
    ...
    received_zc_symbol_6 = signal(received_zc_symbol_6_start_index : received_zc_symbol_6_start_index + ofdm_symb_len + cplen -1);

    % Remove cyclic prefixes

    % signal_without_extended_cp = [signal(cplen_extended-cplen+1 : end - ofdm_symb_len - cplen_extended); signal(end - ofdm_symb_len - cplen + 1: end)]
    % signal_without_cp = reshape(signal_without_extended_cp, [], cplen+ofdm_symb_len);
    % signal_without_cp = signal_without_cp(:, cplen+1:end);
    % signal_without_cp = reshape(signal_without_cp, [], 1);

    % received_zc = zc * h (convolution with the impulse response)
    % Find H: the channel impulse response in the frequency domain
    H_1 = (fftshift(fft(received_zc_symbol_4(cplen+1:end))) ./ fftshift(fft(zc_symbol_4(cplen+1:end)))); 
    H_2 = fftshift((fft(received_zc_symbol_6(cplen+1:end))) ./ fftshift(fft(zc_symbol_6(cplen+1:end)))); 
...
function estimated_signal = fix_channel_estimation(signal, zc_symbol_4, zc_symbol_6)
    ofdm_symb_len = 1024;
    cplen = 72;
    cplen_extended = 80;
    num_of_ofdm_symbols = 9;
    ...
    % % extended_H_1 = (fft(received_zc_symbol_4, ofdm_symb_len + cplen_extended) ./ fft(zc_symbol_4, ofdm_symb_len + cplen_extended)); 
    % % extended_H_2 = (fft(received_zc_symbol_6, ofdm_symb_len + cplen_extended) ./ fft(zc_symbol_6, ofdm_symb_len + cplen_extended)); 
    % % extended_H = (extended_H_1 + extended_H_2) / 2;
    % % 
    % % H_to_divide_by = repmat(H, num_of_ofdm_symbols-2, 1);
    % % Add the extra cp in the first and last symbols
    % % H_to_divide_by = [extended_H; H_to_divide_by; extended_H];

    H_to_divide_by = repmat(H, num_of_ofdm_symbols, 1);

    % Divide each symbol by the channel impulse response
    estimated_signal_in_freq = fft(signal_without_cp) ./ H_to_divide_by;
    ...
    % 
    % % Add cyclic prefix
    % estimated_signal = reshape(num_of_ofdm_symbols ,ofdm_symb_len);
    % estimated_signal = [estimated_signal(end-cplen:end), ]
