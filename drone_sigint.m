...
%% open signal
raw_smaple_base = 'C:\Users\eladm\Desktop\drone sigint\raw_samples-20260423T111818Z-3-001\raw_samples\';
signal_path = [raw_smaple_base,'raw_samples_12_Feb_2026_13_29_59_531_fs_10MHz.32fc'];
...
%%
[freq_offset,phase_offset] = find_freq_phase_offset(signal,zc_seq_sym_4,zc_seq_sym_6,fs);
freq_phase_synced_signal = freq_phase_sync_signal(signal,phase_offset,freq_offset,fs);
...
demod2_8 = demod2_8(cp_len+1:end,:);
demod2_8 = fft((demod2_8));
demod2_8 = demod2_8(:);

demod2_8_no_zc = [demod2_8(1:3*ofdm_len) ; demod2_8(4*ofdm_len+1:5*ofdm_len) ; demod2_8(6*ofdm_len+1:end)];
demod9 = fft((sig9));
...
function [freq_offset,phase_offset] = find_freq_phase_offset(signal,zc_seq_sym_4,zc_seq_sym_6,fs)
...
phase_diff = angle(corr2(samp2).*conj(corr1(samp1)));
...
function synced_signal = freq_phase_sync_signal(signal,phase_offset,freq_offset,fs)
N = length(signal);
t_val = (0:N-1).' ./fs;
fix_sig = exp(1j*(-2*pi*freq_offset.*t_val - phase_offset)).*t_val;%signal phase and freq offsets exp signal
synced_signal = signal.*fix_sig;
end
